...
function openBillForm(bill = null) {
  ...
  openModal(`
    ...
    <form id="bill-form" class="grid cols-2" style="gap:12px">
      ...
      ${State.paymentMethods.length ? `<div class="field"><label>Metodo di pagamento</label><select class="select" name="payment_method_id">${optList({ "": "—", ...Object.fromEntries(State.paymentMethods.filter(pm => pm.active).map(pm => [pm.id, `${pm.label}${pm.last4 ? " ••" + pm.last4 : ""} (${memberName(pm.user_id)})`])) }, b.payment_method_id || "")}</select></div>` : ""}
